...
fn task1() -> usize {
    let file = std::fs::File::open("./input.txt").expect("file open failed");
    let lines = std::io::BufReader::new(file).lines();

    let ids = lines.map(|line| line_to_id(line.unwrap()));
    ids.max().unwrap()
}
...
fn task2() -> usize {
    ...
    let mut ids: Vec<usize> = lines.map(|line| line_to_id(line.unwrap())).collect();
    ids.sort();
    for i in 0..ids.len() {
        let this =ids[i];
        let next = ids[i+1];
